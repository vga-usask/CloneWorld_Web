...
{
    public static class CloneColorFiller
    {
        public static readonly List<string> Colors = GetColorList();

        public static void FillClassColorsForClones(List<CloneInstance> cloneList, List<CloneClass> classList)
        {
            Console.WriteLine("Filling class colors for clones...");
            foreach (var clone in cloneList)
            {
                clone.ClassColor = Colors[classList.FindIndex(
                    clazz => clazz.Type.ToString() == clone.ClassType && clazz.Id.ToString() == clone.ClassId
                ) + 1];
            }
        }

        public static void FillDirectoryColorsForClones(List<CloneInstance> cloneList)
        {
            Console.WriteLine("Filling directory colors for clones...");
            var directorySet = new HashSet<string>();
            foreach (var clone in cloneList)
            {
                var path = Path.GetDirectoryName(clone.FilePath);
                directorySet.Add(path);
            }

            var directoryList = new List<string>(directorySet);
            foreach (var clone in cloneList)
            {
                clone.DirectoryColor = Colors[directoryList.FindIndex(
                    directory => directory == Path.GetDirectoryName(clone.FilePath)
                ) + 1];
            }
        }
